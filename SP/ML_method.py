...
def windowed_dataset(series, window_size, batch_size, shuffle_buffer):
  ...
  dataset = dataset.batch(batch_size).prefetch(1) # 最后一批数量可能不足
  return dataset
...
# plt.semilogx(lrs, history.history["loss"]) # x坐标取对数 , [ semilogy]
...
# print('model predict: ',model.predict(series[:20][np.newaxis]))
# forecast=forecast[split_time-window_size:]
print('Forecast: ',forecast[0],len(forecast))
